...
namespace kas { namespace core
{
    ...
    struct core_relax
    {
        ...
        void relax()
        {
            ...
            // now relax sections
            core_section::for_each([&](auto& section)
                {
                    if (trace)
...
#if 0
                segment.for_frags([&](auto& frag)
                    {
                        relax_frag(frag, fuzz);
                    });
#else
    ...
    template <typename C>
    decltype(auto) do_relax(C& c, std::ostream *trace = nullptr)
    {
        return core_relax<C>{c, trace}.relax();
    }
