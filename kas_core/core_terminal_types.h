...
// move, etc, a `symbol_ref` as the `int` that it is.
//
...
// declare objects stored in `expr_t`
...
// NB: c++ promotion rules will convert smaller unsigned to larger signed

// type to hold frag offset & frag size
using frag_offset_t = offset_t<uint16_t>;

// type to hold frag/seg/section/etc size
using size_offset_t = offset_t<uint64_t>;

// type to hold expression calculations
using expr_offset_t = offset_t<int64_t>;

// these definitions need to be compatible with opcode_data::fixed_t;
