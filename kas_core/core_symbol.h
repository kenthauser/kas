...
namespace kas::core
{
    ...
    struct core_symbol : kas_object<core_symbol, symbol_ref> {
    ...
    template <typename OS>
    void core_symbol::print(OS& os) const
    {
        os << "[" << s_name << "(index: " << index() << ")]";
    }

    template <typename OS>
    void core_symbol::dump(OS& os)
    {
    ...
    namespace detail 
    {
        ...
        static bool is_emitted(core_symbol const& s) 
        {
#if 0
            switch (s.binding()) {
            case STB_INTERNAL:
    ...
    // possibly need a more general `for_each_if`
    template <typename FN, typename...Ts>
    void core_symbol::for_each_emitted(FN fn, Ts&&...args)
    {
        for_each([&fn](core_symbol& s, auto&&...fn_args)
            {
