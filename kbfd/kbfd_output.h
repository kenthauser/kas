#ifndef KBFD_KBFD_OUTPUT_H
#define KBFD_KBFD_OUTPUT_H
...
struct kbfd_format
{
    ...
    void init_from_kas_core()
    {
        ...
        auto symtab_p = new ks_symbol();
        kbfd_section::symtab_index = symtab_p->section.index();
    ...
    void write(std::ostream& os)
    {
        ...
        header.e_shnum      = kbfd_section::num_objects() + 1;
        ...
        kbfd_section::for_each([&os, &cvt=cvt](auto& s)
            {
                if (s.s_header.sh_type == SHT_NOBITS)
                    return;
                if (s.padding)
        ...
        kbfd_section::for_each([&os, &cvt=cvt, &header=header](auto& s)
            {
    ...
    std::vector<ks_data> data_sections;
    ks_string *sh_string {};
};
