...
template <typename RT, int RADIX, int SEP_CHAR = 0, typename It>
...
template <typename RT, int RADIX>
auto decode_int = [](auto& ctx)
{
    auto& range = _attr(ctx);
    auto  it = range.begin();
    _val(ctx) = _str2int<RT, RADIX, cpp14_sep>(it, range.end());
};
...
auto const c_fixed_p_def = lexeme[
    // handle hex(0x*) binary(0b*) octal(0*) & decimal(*) formats...
      parse_int("0x", xdigit,       decode_int<T, 16>)
    | parse_int("0b", char_("01"),  decode_int<T,  2>)
    | parse_int("0",  char_("0-7"), decode_int<T,  8>)
    | parse_int(eps,  digit,        decode_int<T, 10>)
...
auto const c_string_p_def = lexeme[
    // these parsers can "throw" for invalid unicode sequences
      parse_str(eps,       decode_char_seq<T>{1, false})
    | parse_str(lit("L"),  decode_char_seq<T>{2, false})
    | parse_str(lit("u8"), decode_char_seq<T>{1, true })
    | parse_str(lit("u"),  decode_char_seq<T>{2, true })
    | parse_str(lit("U"),  decode_char_seq<T>{4, true })
    ];

template <typename T>
struct c_int_rule : rule<struct _, T> 
{
    template <typename Iterator, typename Context, typename Attribute_>
    bool parse(Iterator& first, Iterator const& last
      , Context const& context, unused_type, Attribute_& attr) const
    {
        return c_fixed_p_def<T>.parse(*this, first, last, context, attr);
    }
};
#endif
