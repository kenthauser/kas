#ifndef KAS_ELF_Z80_Z80_ELF_H
#define KAS_ELF_Z80_Z80_ELF_H

#include "kas_core/emit_object.h"      // for `obj_format` template
#include "elf/elf_format_aout.h"
#include "elf/elf_format_aout_write.h"

namespace kas::elf::z80
{
...
struct z80_aout : elf_format_aout<std::endian::little>
{
    using base_t = elf_format_aout<std::endian::little>;
    using elf_use_rela = std::false_type;

    z80_aout()
        : base_t(relocs, EM_Z80)        // EM_Z80 is `elf` name
    {
...
{
    template <> struct obj_format<void> : meta::id<elf::z80::z80_aout> {};
}
