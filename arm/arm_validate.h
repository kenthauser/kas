...
struct val_reg : tgt::opc::tgt_val_reg<val_reg, arm_mcode_t>
{
    ...
    fits_result ok(arm_arg_t& arg, uint8_t sz, expr_fits const& fits) const override
    {
    ...
    {
        arg.reg = {RC_GEN, static_cast<uint16_t>(value)};
    }
};
...
struct val_shift : arm_mcode_t::val_t
{
    ...
    {
        if (arg.shift.is_reg)
            return (arg.shift.type << 5) | (arg.shift.reg << 8) | 0x10;    
...
// unsigned validators
VAL_GEN(U16       , val_range, 0, 0, (1<<16) - 1);
VAL_GEN(U12       , val_range, 0, 0, (1<<12) - 1);
