#ifndef KAS_M68K_OPC_EMIT_H
#define KAS_M68K_OPC_EMIT_H
...
namespace kas { namespace m68k { namespace opc
{
    ...
    struct m68k_opc_emit : opcode
    {
        OPC_INDEX();
        const char *name() const override { return "M68K_EMIT"; }

        m68k_opc_emit() = default;

        template <typename Insn, typename M68K_Inserter>
        m68k_opc_emit(Insn const& insn, M68K_Inserter& inserter
                       ...
                       , args_t&& args)
        {
            ...
            m68k_insert_args(inserter, std::move(args), op_p, insn.fmt_idx, false);
