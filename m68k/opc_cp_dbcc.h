#ifndef KAS_M68K_M68K_OPC_CP_DBCC_H
#define KAS_M68K_M68K_OPC_CP_DBCC_H
...
struct m68k_opc_cp_dbcc: m68k_stmt_opcode
{
    using base_t::base_t;

    OPC_INDEX();
    const char *name() const override { return "M68K_CP_DBCC"; }
                 ...
                 ) override
    {
        ...
        insn_size  = {6, 14};          // include pseudo-code if branch distance exceeded
    ...
    void emit(Iter it, uint16_t cnt, core::emit_base& base, core::core_expr_dot const& dot) override
    {
        ...
        uint16_t code1 = (opcode.code() >> 16) | reg_num;
        uint16_t code2 = opcode.code();

        // if "fits" just do word displacement
        if (size_p->max == 6)
        {
            // displacement from "dot + 2"
            base << code1 << code2 << core::emit_disp(dot, 2, 2) << dest;
                
        } else
        {
            base << code1 << code2 << core::set_size(2) << 4;   // 6: DBcc: jump over branch
