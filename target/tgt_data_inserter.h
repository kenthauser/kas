...
struct tgt_data_inserter
{
    ...
    // make sure `n` chunks available in current "segment"
    void reserve(unsigned cnt)
    {
        if (n == 0)
            ci.reserve(cnt);
        else if (n < cnt)
    ...
    value_type* operator()(int i, int size);
...
template <typename value_type, typename Inserter>
value_type *tgt_data_inserter<value_type, Inserter>::operator()(int i, int size) 
{
...
struct tgt_data_reader 
{
    ...
    {
        n = sizeof(fixed.fixed) / sizeof(value_type);
        p = static_cast<value_type *>(&fixed.fixed);
    }
