...
// NB: X3 doesn't tag objects generated by semantic actions, 
// NB: thus generate intermediate `raw*` object, then tag
auto const raw_all_labels = rule<class _, bsd_stmt_label> {}
        = all_labels[bsd_stmt_label()];

// Boilerplate to instantiate `label` parser
stmt_label_x3 stmt_label {"bsd_label"};
auto const stmt_label_def = raw_all_labels;
...
// NB: X3 doesn't tag objects generated by semantic actions
// NB: generate intermediate `raw*` object, then tag
auto const raw_stmt_comma = rule<class _comma, bsd_stmt_pseudo> {}
        = (comma_op_x3 > comma_args)[bsd_stmt_pseudo()];
auto const raw_stmt_space = rule<class _space, bsd_stmt_pseudo> {}
